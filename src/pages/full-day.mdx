
# Hello, content community!

With a truly broken heart we received the news about the soap! conference not
happening this year. But once the first wave of sadness has passed, we wiped
away our tears, blew off the dust from our keyboards, and decided we want to
meet all of you this year anyway!

Join us at **#ContentDay**!

- **What?** A full-day MeetContent event, with workshops and presentations.
- **When?** June 8, 2024 
- **Where?** Institute of English Studies at Jagiellonian University, Kraków

<br/>

{/* ## Agenda

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs>
<TabItem
    value="workshops"
    label="Workshops"
    default>

Coming soon!
</TabItem>

<TabItem
    value="presentations"
    label="Presentations"
    default>

Coming soon!

  </TabItem>
</Tabs>  */}


## Sign up

Are you as excited as we are? Join us for workshops and presentations! 

### Workshops

Our workshops run in parallel at 10AM-12PM, which means you can attend just one of them.
> The number of spots is limited and on a first-come, first-serve basis.

<details>
  <summary>**Design (thinking) a doc site** with Paweł Kowaluk</summary>
  <div>
    **About the workshop** <br/>
    Learn how to apply design thinking to creating a doc website. We will create a persona and use that to create the sitemap for our documentation site. <br/>
    **About the trainer** <br/>
    Paweł Kowaluk has been in the tech writing industry since 2008. He has worked as a tech writer, manager, project manager, consultant, salesman, instructor, and doc tools developer. Currently, he does technical writing for Guidewire Software and serves as a board member at ITCQF. Additionally, Paweł explores the technical side of content creation as a co-founder and host of the https://techwriterkoduje.pl podcast. <br/>
    **What to bring**: For this workshop you will need a laptop.
    <hr/>
    **Available spots**: 12 <br/>
    **Sign up form**: https://forms.gle/cM5NFSK2Tyc9wmxe9
  </div>
</details>

<details>
  <summary>**Writing and designing** with Paweł Chłodnicki</summary>
  <div>
    **About the workshop** <br/>
    We'll start with a blank page and leave the room with your first mobile app design. Along the way, you'll learn how words shape digital experiences and what tools are used to achieve that. <br/>
    **About the trainer** <br/>
    Paweł Chłodnicki is a T-shaped Content Designer, powered by content-first approach, Product Design skills, and entrepreneur history. He is especially interested in scaling style guides, design systems and patterns. Paweł loves sharing knowledge and networking! After work, Paweł is a family guy, book worm, coffee geek, motorbike rider, and a rebel 🤘🤘 <br/>
    **What to bring**: You don't need a laptop, we will work with traditional resources like post-it notes :) 
    <hr/>
    **Available spots**: 20 <br/>
    **Sign up form**: https://forms.gle/cM5NFSK2Tyc9wmxe9
  </div>
</details>

### Presentations

Missed the registration for the workshop? Don't like waking up in the morning? No worries, you can still attend our presentations!
Sign up at: https://forms.gle/ZQnfTGCLJr3MwM7s9

<br/>

## Let's stay in touch

Our lovely content community and beautiful Cracow in June, can it get any better than
that? Well, who knows? We might have a few surprises up in our sleeve 🤫
More details coming soon, so keep this page bookmarked!

<hr/>

Want to reach out to us directly? Drop us a message at krk.meetcontent@gmail.com.

<hr/>
<br/>

## About our sponsors

**#ContentDay** is sponsored by [Apify](https://apify.com/) and [Box](https://www.box.com)!

Apify is the only full-stack cloud platform where developers can build, deploy, 
and publish web scraping, data extraction, and web automation tools. Use 1,500+ 
ready-made tools from Apify Store or create your own and earn passive income from 
sharing them with other users. Apify is also behind open-source web scraping library Crawlee.

Box dates back to 2005 when it started making it easy to access information from
anywhere and collaborate with anyone. Today, Box is proud to call 97,000
companies and 68% of the Fortune 500 our customers — but that’s just the start.
As work continues to evolve, Box stays focused on delivering innovation to
organizations across the globe and blowing their customers' minds every day.

<br/>

## FAQ

<details>
  <summary>Is **#ContentDay** a free event?</summary>
  <div>
    Yes! The event is completely free of charge but requires registration.
  </div>
</details>

<details>
  <summary>Do I have to attend the entire event?</summary>
  <div>
    **#ContentDay** consists of two parts: 
    - workshops in the morning, 
    - presentations in the afternoon. 

    You can participate in both parts or only one of them.
    Just remember to register!
  </div>
</details>

<details>
  <summary>Is there a limited number of spots?</summary>
  <div>
     For workshops, the number of spots is limited and on a first-come, first-serve basis.
     The registration will open as soon as we announce the agenda.
  </div>
</details>

<details>
  <summary>How can I present a topic at **#ContentDay**?</summary>
  <div>
    We'd love to hear from you, [use this form to propose a presentation or a
    workshop](https://forms.gle/aKPDg8S6PFsdRRsq8).
  </div>
</details>